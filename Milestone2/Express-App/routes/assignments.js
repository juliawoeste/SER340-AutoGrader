...
assignmentsRouter.route('/:assignmentsId')
.get((req,res,next)=>{
    // 4- find by id 
...
.put((req, res, next)=>{
  ...
  assignments.findByIdAndUpdate(req.params.assignmentsId, 
      {$set:req.body}, 
      {new: true}, 
      (err, assignments) => {
          if (err) throw err;
...
.delete((req, res, next)=>{
    ...
    assignments.findByIdAndRemove(req.params.assignmentsId,  (err, assignments)=>{        
              if (err) throw err;
              res.json(assignments);
      });
});

assignmentsRouter.route('/:assignmentsId/submissions')
.get((req, res, next)=>{
    assignments.findById(req.params.assignmentsId,  (err, assignments)=>{
        if (err) throw err;
...
assignmentsRouter.route('/:assignmentsId/submissions/:submissionsId')
